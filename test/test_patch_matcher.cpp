...
TEST_CASE("PatchMatcher on simple translation") {
    ...
    typedef PatchMatcher<WholeImagePatches, WholeImagePatches, DistanceType> MatcherType;
    ...
    SECTION("Converges in only one iteration if correct offset is manually set") {
        ...
        Mat_<Vec2i> offset_map = matcher.get_offset_map()(cv::Rect(2, 2, 508, 508));
        Mat_<Vec2i> expected(508, 508, Vec2i(0, 10));
        Mat_<int> expected_distances(508, 508, 0);

        require_matrix_equal(matcher.get_distance_map()(cv::Rect(2, 2, 508, 508)), expected_distances);
